...
def input_field(
    ...
    return rx.el.div(
        ...
        rx.el.input(
            ...
            class_name=rx.cond(
                RecordState.errors[name] != "",
                "mt-2 w-full rounded-lg border border-danger-300 bg-white px-3 py-2 text-sm font-medium text-neutral-900 placeholder:text-neutral-400 focus:border-danger-500 focus:ring-2 focus:ring-danger-200 outline-hidden",
                "mt-2 w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-medium text-neutral-900 placeholder:text-neutral-400 focus:border-brand-500 focus:ring-2 focus:ring-brand-200 outline-hidden",
            ),
        ),
        field_error(name),
        class_name="flex w-full flex-col",
...
def notes_field() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.textarea(
            ...
            class_name="mt-2 w-full resize-none rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-medium text-neutral-900 placeholder:text-neutral-400 focus:border-brand-500 focus:ring-2 focus:ring-brand-200 outline-hidden",
        ),
        class_name="flex w-full flex-col",
    )
...
def room_subform_dialog() -> rx.Component:
    return rx.cond(
        ...
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    rx.el.h3("Habitación", class_name="text-base font-semibold text-neutral-900"),
                    rx.el.button(
...
def tenant_subform_dialog() -> rx.Component:
    return rx.cond(
        ...
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    rx.el.h3("Inquilino", class_name="text-base font-semibold text-neutral-900"),
                    rx.el.button(
                        rx.icon("x", class_name="h-4 w-4"),
                        type="button",
    ...
    return rx.el.form(
        ...
        rx.el.div(
            rx.el.button(
                ...
                class_name="rounded-lg border border-neutral-300 bg-white px-4 py-2 text-sm font-semibold text-neutral-700 transition-colors hover:bg-neutral-50",
            ),
            rx.el.button(
    ...
    return rx.cond(
        ...
        rx.el.div(
            rx.el.div(
                ...
                class_name="max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-neutral-200 bg-white",
            ),
            class_name="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-neutral-900/40 p-4 py-8 backdrop-blur-sm",
        ),
        rx.el.div(),
    )
